...
while i < len(fractions_p):
    ...
    if q_match:
        ...
        if opts and ans_letter:
            ...
            fractions_qs.append({
                "id": f"frac_{q_num}_{current_level}",
                "level": current_level,
                "topic": "Fractions",
                "question": q_text,
                "options": opts[:4],
                "correctIndex": c_idx if c_idx < len(opts) else 0,
                "explanation": f"Topic: Fractions ({current_level.upper()}). Correct answer is Option {ans_letter}: {opts[c_idx] if c_idx < len(opts) else opts[0]}."
            })
        i = j - 1
    i += 1

print(f"Parsed Fractions Qs: {len(fractions_qs)}")

# 2. PARSE FACTORS / MULTIPLES / LCM / HCF
...
print(f"Extracted Answer Key Count for Factors: {len(ak_map)}")

factors_qs = []
current_level = "primary"

for idx, p in enumerate(factors_p):
    if "ANSWER KEY" in p.upper() and idx > 100:
        break
    ...
    if q_match:
        ...
        if idx + 1 < len(factors_p):
            ...
            if len(parts) >= 9:
                ...
                c_idx = ord(ans_letter) - ord("A")
                factors_qs.append({
                    "id": f"fact_{q_num}_{current_level}",
                    "level": current_level,
                    "topic": "Factors, Multiples, LCM & HCF",
                    "question": q_text,
                    "options": opts[:4],
                    "correctIndex": c_idx if c_idx < len(opts) else 0,
                    "explanation": f"Topic: Factors, Multiples, LCM & HCF ({current_level.upper()}). Correct answer is Option {ans_letter}: {opts[c_idx] if c_idx < len(opts) else opts[0]}."
                })

print(f"Parsed Factors/LCM/HCF Qs: {len(factors_qs)}")

all_qs = fractions_qs + factors_qs
print(f"Total Combined Questions: {len(all_qs)}")

# Group Summary by Level & Topic
summary = {}
for q in all_qs:
    lvl = q["level"]
    tpc = q["topic"]
    if lvl not in summary:
        summary[lvl] = {}
    if tpc not in summary[lvl]:
        summary[lvl][tpc] = 0
    summary[lvl][tpc] += 1

print("\n=== SUMMARY BREAKDOWN BY GRADE LEVEL AND TOPIC ===")
print(json.dumps(summary, indent=2))

# Write to questions-data.js
js_content = f"export const SAMPLE_200_QUIZZES = {json.dumps(all_qs, indent=2)};"
with open("questions-data.js", "w") as f:
    f.write(js_content)

print("\nSuccessfully updated questions-data.js with parsed DOCX questions!")
